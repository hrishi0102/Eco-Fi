...
export function Donated({ nft }: Props) {
  ...
  async function stakeNFT(id: string) {
    ...
    const isApproved = await donationContract?.erc1155.isApproved(
      address,
      STAKING_ADDRESS
    );

    if (!isApproved) {
      await donationContract?.erc1155.setApprovalForAll(STAKING_ADDRESS, true);
    }
